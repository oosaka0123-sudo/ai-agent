...
## theme.json の status について

`guides/<テーマID>/theme.json` の `ais` 配列にある各AIの `status` は、
`web/assets/js/guides-data.js` が表示ラベルに変換します。以下のいずれかを使ってください。

- `"not-started"` → 「未執筆」（まだ本文を書いていない）
- `"in-progress"` → 「執筆中」
- `"completed"` または `"published"` → 「公開中」（本文を書き終えた）

本文を書き終えたら、必ず自分の担当分の `status` をこのいずれかに更新してください
（`theme.json` はサイト共通のメタデータなので、この更新は自分のファイルの範囲内の
変更として行ってかまいません）。
