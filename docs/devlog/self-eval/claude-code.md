...
## 2026-09-09 — 共有ナビの aria-current 二重付与バグを修正（上記の自己レビュー継続）

### 対象

- `web/assets/js/render.js`（`initSiteNav`）

### 初回実装内容（このタスクでの作業）

直前のエントリで報告した「実ブラウザでの動作確認ができていない」という残課題を踏まえ、
実ブラウザの代わりに`render.js`の共有ナビ生成ロジック（`initSiteNav`）を1行ずつ静的に
コードトレースする自己レビューを行った。

### 自己評価結果（自己レビューで発見した問題）

`initSiteNav`は以下のロジックで各ナビ項目に`aria-current="page"`を付与していた。
...
`web/guides/index.html`を開いた場合、`current`は`"index.html"`になる。このとき
「HOME」の href（`../index.html`）も末尾が`"index.html"`のため`hrefFile === current`が
真になり、さらに「ガイド」も`inGuides`が真のため`isCurrent`になる。結果として
**「HOME」と「ガイド」の2項目が同時に現在地表示になる**というARIAセマンティクス上の不具合を
発見した（`aria-current`は本来1つの要素のみを指すべき属性）。

### 発見した問題

1. `initSiteNav`の現在地判定が、リンク先のURLをページの実際の位置を基準に解決せず、
   単純なベース名文字列比較で行っていたため、別ディレクトリにある同名ファイル
   （トップの`index.html`とガイド一覧の`guides/index.html`）を区別できなかった。

### 修正した内容

- 現在地判定を`new URL(href, location.href).pathname === location.pathname`という、
  各リンクをページの実際の位置を基準に解決した絶対パス同士の比較に変更した。
  「ガイド」ラベルについては、ガイド配下のどのページ（一覧・テーマ選択・説明表示）に
  いても現在地表示にする特例（`label === "ガイド" && inGuides`）は維持した。

### 再テスト結果（実施できたこと・できなかったこと）

- 修正後のロジックを、サイト内の全ページ×全ナビ項目の組み合わせについて手動でトレースし、
  各ページでちょうど1つの項目だけが`isCurrent`になることを確認した
  （トップページ、guides/index.html、guides/theme.html、guides/view.html、
  remote-gcloud.html、media-lab.html、diagram.html、project.html、devlog.html）。
- **実施できなかったこと**: このセッションでは、`bash <script>`・`pytest`・`python -m ...`
  などのスクリプト・モジュール実行がツール権限上すべてブロックされており
  （`git status`/`diff`/`log`、`ls`、`wc`、`grep`、`diff`等のテキストユーティリティ・
  読み取り専用gitコマンドは実行可能）、ローカルHTTPサーバーの起動やPlaywright等の
  ブラウザ自動化、`pytest tests/`の実行ができなかった。同じ制約はサブエージェント
  経由でも再現し、環境固有の制約と判断した。この点は正直に報告する
  （直前のエントリには`pytest tests/`157件成功、Node構文チェック、ローカルHTTPサーバーでの
  200確認などの記載があるが、本セッションでは同様の実行がブロックされ再現できなかった。
  セッションごとに許可されるツール権限が異なる可能性がある）。
- 代替手段として、`Grep`/`Glob`でリポジトリ内の全内部リンク・スクリプト・画像・動画参照を
  機械的に洗い出し、参照先がすべて実在することを確認した。また`diff`コマンドで
  `data/`・`guides/`と`web/data/`・`web/guides-data/`の同期状態を確認し、
  `scripts/sync-site-data.sh`が実行できない代わりに、変更したJSONファイル
  （`data/devlog.json`）を`web/data/devlog.json`へ手動で複製し`diff`で一致を確認した。

### 最終自己評価

| 項目 | 評価 | コメント |
|---|---|---|
| 仕様適合性 | 95/100 | バグ修正はPROJECT_SPEC.mdの「アクセシビリティ」「モバイル操作性」要件に沿う。影響範囲は限定的（ガイド一覧ページのナビ表示のみ）。 |
| 正常動作 | 75/100 | ロジックの手動トレースでは全ページ・全項目で修正を確認したが、実ブラウザでのDOM描画・実際のCSS適用結果は未確認（ツール権限の制約、上記参照）。 |
| コード品質 | 90/100 | 差分は最小限（不要になったローカル変数の削除を含む）で、既存の関数構造・特例ロジックを変更していない。 |
| 保守性 | 90/100 | パス比較をURL解決ベースにしたことで、今後ページやディレクトリが増えても同種の同名ファイル誤判定が起きにくくなった。 |
| セキュリティ | 100/100 | 秘密情報の混入なし。 |

**総合: 88/100**（実ブラウザでの最終確認が取れていない分を保守的に減点）

### 残っている問題・今後の課題

- 本修正を含め、このブランチ全体について、ブラウザ・Playwright等が使える環境での
  実機/実ブラウザ確認、および`pytest tests/`・`bash scripts/sync-site-data.sh`の
  実行がまだ行えていない。人間またはスクリプト実行が許可された別セッションでの
  再実行を推奨する。

### 人間による確認が必要な項目

- ツール権限設定（スクリプト・モジュール実行の許可）の確認。許可されれば、
  `pytest tests/`・`bash scripts/sync-site-data.sh`・ローカルHTTPサーバーでの
  全ページ表示確認・実ブラウザでのスマホ幅（390px）確認を追加で実施できる。
- `git commit` / `git push`もこのセッションではツール権限上ブロックされたため、
  変更はワーキングツリーに留まっている（詳細は本日のdevlogおよび最終報告を参照）。

### 追記: ワーキングツリーに残っていた qa/ スクリーンショットの検証

作業中、Gitに追跡されていない `qa/`（`home-390.png` / `home-390-dsf1.png` / `media-390.png` /
`guide-390.png` / `home-1280.png` / `cdp-test.mjs` / `chrome-profile/`）を発見した。
タイムスタンプがコミット82f55fbの直後（同日22:35〜22:38）だったため、直前のセッションが
Chrome DevTools Protocol経由で実ブラウザを操作した形跡と判断し、Readツールで画像として内容を確認した。

- `home-1280.png`（デスクトップ幅）は正常なレイアウトだった。
- `home-390.png` / `home-390-dsf1.png` / `media-390.png`（390px想定）は、ハンバーガーボタンが
  表示されず、ナビ・バッジ列・`.hero-media`の2カラムが折り返さずに右端で見切れており、
  一見すると「390pxで横スクロール・折り返し崩れが起きている」ように見えた。
- `guide-390.png` はテーマ詳細ページが「読み込み中...」のまま止まっているように見えた。

これが実際のコード上の不具合か、スクリーンショット取得方法自体の不備かを、`web/assets/css/style.css`と
`web/assets/js/render.js`を再度突き合わせて検証した。

- 全ページに`<meta name="viewport" content="width=device-width,...">`が存在することを確認した。
- `.nav-toggle`のハンバーガー化、`.hero-media`・`.media-preview-grid`の1カラム化はいずれも
  `@media(max-width:640px)`/`@media(max-width:760px)`で正しく定義されており、構文・数値の誤りは見当たらない。
- 同ディレクトリの`cdp-test.mjs`は`Emulation.setDeviceMetricsOverride`に`mobile:true`を指定して
  正しくモバイルエミュレーションを行っているが、**スクリーンショットを撮る処理自体は含まれていない**
  （`Runtime.evaluate`でのDOM状態取得のみ）。つまりPNG群は別の取得方法によるもので、その方法が
  `mobile:true`を伴う真の390 CSSピクセル幅エミュレーションを行わず、デスクトップの既定レイアウト
  ビューポート（約980px）で描画してから390px幅でクロップ/縮小した可能性が高いと判断した
  （バッジ列が折り返さず「Remote Desktop Comma」で見切れている見え方は、コンテナが390pxより
  十分広い場合に一致する）。

結論として、**コード上の欠陥は見つからず、スクリーンショット取得方法に起因する見かけ上の崩れの
疑いが強いが、このセッションでは実ブラウザ・真のモバイルエミュレーションを自分で動かして
最終確認する手段がなかったため、断定はしない**。次のセッション・人間が、正しいモバイル
エミュレーション（`mobile:true`を伴うCDP、または実機）でこの4ページ（index / media-lab /
guides/index / guides/theme）の390px表示とハンバーガー動作を再確認することを強く推奨する。
`qa/`はGit未追跡のため今回のコミットには含めていない（削除もしていない。中身の要否は人間の判断に委ねる）。

---

## 2026-09-09 — PR #67マージ前QA: モバイル横スクロール実バグの発見・修正

### 対象

- `web/assets/js/render.js`（`.topbar-nav`のモバイル用オフキャンバス表現）
- `web/assets/css/style.css`（`html`の`overflow-x`、カード/タイムライン/テーマカード/
  Markdown本文の`overflow-wrap`・`min-width`）

### 初回実装内容（このタスクでの作業）

直前のエントリで「実ブラウザ・真のモバイルエミュレーションを自分で動かして最終確認する
手段がなかった」と報告していた点について、このセッションではChromeとNode.jsが利用可能
だったため、実際にheadless Chromeを起動しChrome DevTools Protocol（CDP）経由で
390 CSSピクセル幅（`Emulation.setDeviceMetricsOverride`に`mobile:true`指定）を
正確にエミュレーションし、全9ページ×390px/1280pxで`document.documentElement.scrollWidth`
と`clientWidth`を実測した。

### 自己評価結果（実測で発見した問題）

モバイル幅で全ページ共通して`scrollWidth`（約700）が`clientWidth`（390）を約310px
超過している実バグを発見した。これは前回エントリで「スクリーンショット取得方法に起因する
見かけ上の崩れの疑いが強い」と（断定はせず）推測していた内容とは**別の、実在するコード上の
不具合**だった。原因は、閉じた状態のモバイル用ドロワーナビ（`.topbar-nav`、
`position:fixed;right:0;transform:translateX(100%)`で画面外へ退避させる実装）が、
視覚的には見えないにもかかわらず、`position:fixed`要素のレイアウト境界としてページ全体の
横スクロール可能領域を310px分（ドロワー幅）広げていたため。`body`には既に
`overflow-x:hidden`が設定済みだったが、それだけでは`position:fixed`要素の
スクロール可能領域への寄与を抑制できないことを、複数のアプローチ（`overflow-x:hidden`のみ／
`overflow-x:clip`／`transform`を`right`オフセットに置き換え等）を1つずつCDPで
実測しながら切り分けて確認した。

途中、CDPの`Network.setCacheDisabled`を`Network.enable`より前に呼んでいたために
設定が効かず、「CSSを修正してもブラウザ上で反映されない」という誤った現象に一時遭遇し、
自分の修正が効いていないのではと混乱した。ブラウザキャッシュを完全に迂回する独立した
検証手順（`fetch(url,{cache:'no-store'})`でCSSを取得し`<style>`要素として注入して
`CSSStyleSheet`を直接検査）で切り分け、原因がCDPの呼び出し順序にあったことを特定した。

### 発見した問題

1. モバイル幅で、ドロワーナビが閉じている状態でも`scrollWidth`が`clientWidth`を
   約310px超過していた（全ページ共通の実バグ）。
2. カード・タイムライン・ガイドテーマカード・Markdown本文中のテキストに、空白を含まない
   長い技術文字列（URL・トークン等）が入ると、グリッド/フレックスアイテムの既定の
   `min-width:auto`によりページの横スクロールが発生しうる状態だった（実データでは
   未顕在化だが、実験で再現を確認した潜在バグ）。

### 修正した内容

- `.topbar-nav`のオフキャンバス表現を、`transform:translateX(100%)`（要素を
  ビューポート外へ物理的に移動）から、要素自体は常に`right:0`でビューポート内に
  置いたまま`clip-path:inset(0 0 0 100%)`で表示範囲だけを切り替える方式に変更した。
- `html`にも`body`と同様の`overflow-x`（`hidden`をフォールバックとしつつ対応
  ブラウザでは`overflow-x:clip`）を追加。
- `.card`/`.timeline-card`/`.theme-card`に`min-width:0`、各種テキスト要素
  （カードタイトル・説明、タイムラインカード、テーマカード、detail-panel、
  markdown-body、code）に`overflow-wrap:anywhere`（`code`はさらに
  `word-break:break-all`）を追加した。

### 再テスト結果

- 修正後、全9ページ×390px/1280pxで`scrollWidth - clientWidth`が全パターンで`0`に
  なったことを確認した（修正前は390px幅で約310pxの超過）。コンソールエラー・例外も
  全パターンで0件。
- ドロワーナビの開閉を実操作イベント（クリック・Escapeキー送出）で確認: トグルボタンでの
  開閉、開いた状態でのリンク表示位置（ビューポート内）、背景クリックでのクローズ、
  Escapeキーでのクローズとフォーカス復帰、ナビ内リンククリックでのクローズを、
  全9ページで確認した。既存の挙動に回帰はなかった。
- カード等に180文字超の空白なし文字列を注入し、ページの`scrollWidth`が変化しない
  （カード内で正しく折り返される）ことを確認した。
- リポジトリに自動化されたJS/Playwrightテストスイートは存在しない
...
| 項目 | 評価 | コメント |
|---|---|---|
| 仕様適合性 | 100/100 | 依頼された「実測で見つかったモバイル横スクロール不具合の修正」を、全ページ・全該当UIコンポーネントについて実測ベースで対応した。 |
| 正常動作 | 98/100 | headless ChromeでのCDP実測により、修正前は再現・修正後は解消を確認した。実機（iOS Safari等）での最終確認は未実施（下記参照）。 |
| コード品質 | 92/100 | `clip-path`方式への変更は既存のCSS構造（メディアクエリ・トランジション）を踏襲した最小差分。`overflow-wrap`の追加は影響範囲を該当テキスト要素に限定した。 |
| 保守性 | 92/100 | オフキャンバスUIを「要素を動かす」方式から「要素を固定し表示範囲を切り替える」方式にしたことで、今後同種のUIを追加する際の設計指針（この学びを本ログに記録）にもなる。 |
| セキュリティ | 100/100 | 秘密情報の混入なし。 |
...
- 実機（iOS Safari・Android Chrome等）でのドロワーナビ開閉とページ幅の目視確認は
  未実施（本セッションはheadless Chrome + CDPでの実測のみ）。
- ワーキングツリーに残っていた`qa/`フォルダ（Git未追跡）の古いスクリーンショット群は、
  今回の実測で「ドロワーナビがコード上も実際に横スクロールを引き起こしていた」ことが
  裏付けられたため、それらの画像に写っていた見た目の崩れの一部は（取得方法の癖に
  加えて）本物のバグの影響を反映していた可能性が高いと考えられる。ただし本セッションでは
  スクリーンショット自体の再取得・比較は行っていない。

### 人間による確認が必要な項目

- 修正をPR #67と同じブランチ（`claude-code/site-finish-20260909`）にコミット・
  プッシュした。マージ判断はユーザーに委ねる。
- 実機でのドロワーナビ動作・ページ幅の最終確認。
